...
class Jxnewsletter extends Module
{
    ...
    public function __construct()
    {
        ...
        $this->bootstrap = true;
        parent::__construct();
        $this->displayName = $this->l('JX Newsletter');
    ...
    {
        $output = '';
        if (Tools::isSubmit('submitJxnewsletterModule')) {
        ...
        } elseif (Tools::isSubmit('submitJxnewsletterGuestModule')) {
        ...
        if ((int)Validate::isLoadedObject($module = Module::getInstanceByName('ps_emailsubscription')) && $module->isEnabledForShopContext()) {
            ...
            'form' => array(
                ...
                'input' => array(
                    array(
                        ...
                        'name' => 'JXNEWSLETTER_USER_LIVE_MODE',
                        'is_bool' => true,
                    ...
                    array(
                        ...
                        'name' => 'JXNW_USER_VERIFICATION_EMAIL',
                        'is_bool' => true,
                    ...
                    array(
                        ...
                        'name' => 'JXNEWSLETTER_USER_MESSAGE',
                    ...
                    array(
                        ...
                        'name' => 'JXNEWSLETTER_USER_TIMEOUT',
                    ...
                    array(
                        ...
                        'name' => 'JXNEWSLETTER_USER_FTDELAY',
            ...
            'form' => array(
                ...
                'input' => array(
                    array(
                        'type' => 'switch',
                        'label' => $this->l('Live mode'),
                        'name' => 'JXNEWSLETTER_GUEST_LIVE_MODE',
                        'is_bool' => true,
                        'desc' => $this->l('Use this module in live mode'),
                        'values' => array(
                            array(
                    ...
                    array(
                        'type' => 'switch',
                        'label' => $this->l('Verification email'),
                        'name' => 'JXNW_GUEST_VERIFICATION_EMAIL',
                        'is_bool' => true,
                        'desc' => $this->l('Would you like to send a verification email after subscription?'),
                        'values' => array(
                            array(
                    ...
                    array(
                        ...
                        'type' => 'textarea',
                        'desc' => $this->l('Enter a newsletter popup message.'),
                        'name' => 'JXNEWSLETTER_GUEST_MESSAGE',
                        'label' => $this->l('Popup message'),
                        'autoload_rte' => true,
                        'lang' => true
                    ),
                    array(
                        ...
                        'name' => 'JXNEWSLETTER_GUEST_TIMEOUT',
                    ...
                    array(
                        ...
                        'name' => 'JXNEWSLETTER_GUEST_FTDELAY',
                        'label' => $this->l('First delay'),
                        'suffix' => 'hour(s)'
                    ),
                ),
                'buttons' => array(
                    array(
                        ...
                        'title' => $this->l('Save'),
                        'type' => 'submit',
                        'name' => 'submitJxnewsletterGuestModule',
                    ),
                    array(
                        'class' => 'btn btn-default pull-right',
                        'icon' => 'process-icon-refresh',
    ...
    protected function getConfigFormValues($is_guest = false)
    {
        ...
        $sql = 'SELECT *
                FROM '._DB_PREFIX_.'jxnewsletter_settings tms
        ...
        $data['JXNEWSLETTER_'.$type_name.'_LIVE_MODE'] = isset($result['status']) ? $result['status'] : '';
        $data['JXNW_'.$type_name.'_VERIFICATION_EMAIL'] = isset($result['verification']) ? $result['verification'] : '';
        $data['JXNEWSLETTER_'.$type_name.'_TIMEOUT'] = isset($result['timeout']) ? $result['timeout'] : '';
        $data['JXNEWSLETTER_'.$type_name.'_FTDELAY'] = isset($result['ft_delay']) ? $result['ft_delay'] : '';

        foreach (Language::getLanguages(false) as $lang) {
            if (isset($result['id_jxnewsletter'])) {
                $sql_lang = 'SELECT `title`, `content`
                            FROM '._DB_PREFIX_.'jxnewsletter_settings_lang
                            WHERE `id_jxnewsletter` = '.(int)$result['id_jxnewsletter'].'
            ...
            $data['JXNEWSLETTER_'.$type_name.'_TITLE'][$lang['id_lang']] = isset($lang_data['title']) ? $lang_data['title'] : '';
            $data['JXNEWSLETTER_'.$type_name.'_MESSAGE'][$lang['id_lang']] = isset($lang_data['content']) ? $lang_data['content'] : '';
        }
    ...
    protected function preProcess($is_guest = false)
    {
        ...
        if (!Validate::isFloat(Tools::getValue('JXNEWSLETTER_'.$type_name.'_TIMEOUT'))) {
            $errors[] = $this->l('Timeout is not valid.');
        }
        if (!Validate::isFloat(Tools::getValue('JXNEWSLETTER_'.$type_name.'_FTDELAY'))) {
            $errors[] = $this->l('Timeout is not valid.');
        }

        foreach (Language::getLanguages() as $lang) {
            if (!Validate::isGenericName(Tools::getValue('JXNEWSLETTER_'.$type_name.'_TITLE_'.$lang['id_lang']))) {
                $errors[] = $this->l('Invalid title.');
            }
            if (!Validate::isCleanHtml(Tools::getValue('JXNEWSLETTER_'.$type_name.'_CONTENT_'.$lang['id_lang']))) {
    ...
    protected function postProcess($is_guest = false)
    {
        ...
        $sql = 'SELECT `id_jxnewsletter`
                FROM '._DB_PREFIX_.'jxnewsletter_settings
                WHERE `id_shop` = '.(int)$this->context->shop->id.'
                AND `is_guest` = '.(int)$type_val;

        if ($item_id = Db::getInstance()->getValue($sql)) {
            if (!Db::getInstance()->delete('jxnewsletter_settings', '`id_jxnewsletter` = '.(int)$item_id.'')
                || !Db::getInstance()->delete('jxnewsletter_settings_lang', '`id_jxnewsletter` = '.(int)$item_id.'')) {
                return false;
            }
        }

        if (!Db::getInstance()->insert('jxnewsletter_settings', array(
                                                'id_shop' => (int)$this->context->shop->id,
                                                'is_guest' => (int)$is_guest,
                                                'verification' => (int)Tools::getValue('JXNW_'.$type_name.'_VERIFICATION_EMAIL'),
                                                'timeout' => (float)Tools::getValue('JXNEWSLETTER_'.$type_name.'_TIMEOUT'),
                                                'ft_delay' =>(float) Tools::getValue('JXNEWSLETTER_'.$type_name.'_FTDELAY'),
                                                'status' => (int)Tools::getValue('JXNEWSLETTER_'.$type_name.'_LIVE_MODE'),
                                                )) || !$new_id = Db::getInstance()->Insert_ID()) {
            return false;
        }

        foreach (Language::getLanguages(false) as $lang) {
            if (!Db::getInstance()->insert('jxnewsletter_settings_lang', array(
                                                'id_jxnewsletter' => (int)$new_id,
                                                'id_lang' => (int)$lang['id_lang'],
                                                'title' => pSql(Tools::getValue('JXNEWSLETTER_'.$type_name.'_TITLE_'.$lang['id_lang'])),
                                                'content' => pSql(Tools::getValue('JXNEWSLETTER_'.$type_name.'_MESSAGE_'.$lang['id_lang']))
                                                ))) {
                return false;
            }
        }
    ...
    {
        $sql = 'SELECT jxnsl.`title`, jxnsl.`content`
                FROM '._DB_PREFIX_.'jxnewsletter_settings_lang jxnsl
                LEFT JOIN '._DB_PREFIX_.'jxnewsletter_settings jxns
                ON(jxns.`id_jxnewsletter` = jxnsl.`id_jxnewsletter`)
                WHERE jxns.`id_shop` = '.(int)$this->context->shop->id.'
                AND jxnsl.`id_lang` = '.(int)$this->context->language->id.'
                AND jxns.`is_guest` ='.(int)$is_guest;
    ...
    protected function sendVerificationEmail($email, $token)
    {
        ...
        return Mail::Send($this->context->language->id, 'newsletter_verif', Mail::l('Email verification', $this->context->language->id), array('{verif_url}' => $verif_url), $email, null, null, null, null, null, _PS_MODULE_DIR_.'ps_emailsubscription/mails/', false, $this->context->shop->id);
    }
    ...
    {
        if ($this->context->customer->id) {
            $result = Db::getInstance()->insert('jxnewsletter', array(
                'id_user' => (int)$this->context->customer->id,
                'id_shop' => (int)$this->context->shop->id,
                'status' => (int)$status
            ));
    ...
    private function clearCookies($is_guest = false)
    {
        if (!$is_guest) {
            // clear db entires
            if (!Db::getInstance()->delete('jxnewsletter', '`id_shop` = '.(int)$this->context->shop->id.' AND `id_user` != 0')) {
                return false;
            }
            $this->clearCache('user');
        } else {
            // clear db entires
            if (!Db::getInstance()->delete('jxnewsletter', '`id_shop` = '.(int)$this->context->shop->id.' AND `id_guest` != 0')) {
                return false;
            }
    ...
    {
         $this->_clearCache('jxnewsletter_'.$type.'.tpl');
    }

    public function getNewsletterTemplate($type)
    {
        if (!$this->isCached('jxnewsletter_'.$type.'.tpl', $this->getCacheId())) {
            $is_guest = false;
            if ($type == 'guest') {
                $is_guest = true;
            }
        ...
        return $this->display(__FILE__, 'jxnewsletter_'.$type.'.tpl', $this->getCacheId());
    }

    public function hookHeader()
    {
        if ((int)Validate::isLoadedObject($module = Module::getInstanceByName('ps_emailsubscription'))
            ...
            $this->context->controller->addJS($this->_path.'/views/js/jxnewsletter.js');
            $this->context->controller->addCSS($this->_path.'/views/css/jxnewsletter.css');
            $blockingPopup = 0;
            ...
            Media::addJsDef(array(
                'jxnewsletterAjaxController' => $this->context->link->getModuleLink('jxnewsletter', 'ajax'),
                'user_newsletter_status' => $this->checkUser(),
                'blocking_popup' => $blockingPopup,
                'popup_status' => $this->checkLatestShow(),
                'module_url' => $this->context->link->getModuleLink('jxnewsletter'),
