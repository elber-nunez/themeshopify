...
class DayDeal extends ObjectModel
{
    ...
    public static function getDayDealProductsData($id_product)
    {
        ...
        $sql = 'SELECT tdd.*, tddl.`label`
			FROM '._DB_PREFIX_.'jxdaydeal tdd
			LEFT JOIN '._DB_PREFIX_.'jxdaydeal_lang tddl
    ...
    public static function getDayDealProducts()
    {
        ...
        $nbr = Configuration::get('JX_DEAL_DAY_NB');
        $ramdom = (int)Configuration::get('JX_DEAL_DAY_RANDOM');
        if ($ramdom == 0) {
    ...
    {
        $sql = 'SELECT * FROM `'._DB_PREFIX_.'jxdaydeal`
                WHERE `id_product` = '.(int)$id_product.'
