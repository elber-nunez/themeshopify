  ...
  {elseif $input.type == 'filemanager_image'}
    <div class="col-lg-6">
      ...
      {if $input.class == 'hotspot' && isset($fields_value['id_tab']) && $fields_value['id_tab'] != ''}
        <div class="alert-info">{l s='Click this image to store messages' mod='jxlookbook'}</div>
      {/if}

      {if $input.class == 'hotspot' && !isset($fields_value['id_tab']) || $input.class == 'hotspot' && $fields_value['id_tab'] == ''}
        <div class="alert-warning">{l s='For add hotspots first of all select the image and save the tab.' mod='jxlookbook'}</div>
      {/if}
    </div>
  {else}
    {$smarty.block.parent}
