...
{block name='page_content_container'}
  <section id="content" class="page-content page-stores">
    ...
    {foreach $stores as $store}
      <article id="store-{$store.id}" class="store-item card px-xl-3">
        <div class="store-item-container clearfix">
          ...
          <div class="col-md-5 col-sm-7 col-xs-12 store-description">
            ...
            {if $store.note || $store.phone || $store.fax || $store.email}
              <a data-toggle="collapse" href="#about-{$store.id}" aria-expanded="false" aria-controls="about-{$store.id}"><strong>{l s='About and Contact' d='Shop.Theme.Global'}</strong><i class="linearicons-chevron-right"></i></a>
            {/if}
          </div>
