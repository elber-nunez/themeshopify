              ...
              {foreach from=$cart.vouchers.added item=voucher}
                <li class="d-flex align-items-center mt-2 row no-gutters">
                  ...
                  <div class="col-auto ml-2">
                    <a class="close" href="{$voucher.delete_url}" data-link-action="remove-voucher"><span class="linearicons-cross" aria-hidden="true"></span></a>
                  </div>
                </li>
              {/foreach}
            </ul>
          ...
          {block name='cart_voucher_form'}
            <form class="input-group bg-white p-3" action="{$urls.pages.cart}" data-link-action="add-voucher" method="post">
              <span class="input-group-btn">
                <button type="submit" class="btn btn-default">{l s='Add' d='Shop.Theme.Actions'}</button>
              </span>
