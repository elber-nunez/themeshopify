    ...
    {if $group.attributes}
      <div class="product-variants-item">
            ...
            {foreach from=$group.attributes key=id_attribute item=group_attribute}
              <li class="list-inline-item d-inline-block">
                <div class="custom-control custom-checkbox">
                  <label class="custom-control-label{if $group_attribute.selected} active{/if}" {if $group_attribute.html_color_code}style="background-color: {$group_attribute.html_color_code}{if $group_attribute.html_color_code == '#ffffff'}; border: 1px solid{/if}"{/if}{if $group_attribute.texture}style="background-image: url({$group_attribute.texture})"{/if}>
        ...
        {elseif $group.group_type == 'radio'}
          <ul id="group_{$id_attribute_group}" class="radio-links">
            {foreach from=$group.attributes key=id_attribute item=group_attribute}
              <li>
                <label>
                  <input class="custom-control-input" type="radio" data-product-attribute="{$id_attribute_group}" name="group[{$id_attribute_group}]" value="{$id_attribute}"{if $group_attribute.selected} checked="checked"{/if}>
