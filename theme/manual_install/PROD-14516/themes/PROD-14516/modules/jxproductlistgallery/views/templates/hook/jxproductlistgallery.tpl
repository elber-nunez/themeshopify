      ...
      <div class="thumbnails-carousel slide {$smarty_settings.st_type}">
        {if ($smarty_settings.st_type == 'gallery') || ($smarty_settings.st_type == 'slideshow' && $smarty_settings.st_slider_pager)}
          <ol class="carousel-indicators">
            {foreach from=$product.images item=image name=image}
              <li data-slide-to="{$smarty.foreach.image.iteration - 1}"{if $image.cover} class="active"{/if}{if $smarty_settings.st_type == 'gallery'} style="flex: 0 0 calc(100%/{$smarty_settings.st_visible}); -webkit-flex: 0 0 calc(100%/{$smarty_settings.st_visible})"{/if}>
