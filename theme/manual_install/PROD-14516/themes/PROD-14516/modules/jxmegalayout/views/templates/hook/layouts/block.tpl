...
{if $items.module_name == "logo"}
  <a class="logo{if $items.specific_class} {$items.specific_class}{/if}" href="{$urls.base_url}" title="{$shop.name}">
    <img class="img-fluid" src="{$shop.logo}" alt="{$shop.name}">
  </a>
{/if}
{if $items.module_name == "copyright"}
  {if Configuration::get('FOOTER_POWEREDBY')}
    <div class="copyright{if $items.specific_class} {$items.specific_class}{/if}">
